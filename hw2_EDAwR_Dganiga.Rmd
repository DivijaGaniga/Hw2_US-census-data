...
g + geom_histogram(mapping = aes(x = TotalPop),bins =35)
...
ggplot(data = county_MI, aes(x= Citizen, y = ChildPoverty ))+geom_point(aes(colour = Poverty))+facet_wrap(~peninsula)+ ggtitle("Scatterplot of citizenship vs ChildPoverty")+scale_color_gradient(low = "red", high = "blue") +xlab("Total Citizens")
...
ggplot(data = county_MI, aes(x=Income, color = peninsula)) +
  geom_density(data = county_MI)
...
ggplot(data = county_MI, aes(x=White, fill = peninsula, alpha= 0.8)) +
  geom_density(data = county_MI) 
...
ggplot(data = county_MI, aes(x=peninsula, y= citizen_percent))+geom_boxplot()

# Top 15 Counties with highest Income

county_df_2 <- county_df  %>% arrange (desc(county_df$Income)) %>% head(15)
          # arranging in descending order and top 15 counties

county_df_2 %>%  ggplot(aes(x = reorder(County, Income),y = Income, fill = Region))+ geom_bar(stat= "Identity")+coord_flip()+theme(axis.text.y = element_text(size=8, hjust = 1))+  scale_fill_manual(values = c("purple", "blue","pink"))+theme_fivethirtyeight() +
  theme(axis.text.x = element_blank())+geom_text(aes(label = Income))+ggtitle("Top 15 Counties with highest median income")
